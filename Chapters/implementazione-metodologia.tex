...
Plotting the \textsl{Precision-Recall Curve} in figure 19 provides a quick visualization mean to observe the performance drop of the Safety-Monitor, performing well with a \textsl{"dumb"} Controller, while becoming almost disruptive with more trained Controllers.

Looking at the curves, what surprises us about the Monitor is that, even if its performances are not good, its best performance is observed at the \textsl{hardest} difficulty, i.e. the setting where the amount of pedestrians and vehicles in the scenarios is increased. After $C1$, the second best performances are observed with the second hardest difficulty for the Controller, i.e. the setting where the amount of vehicles is increased. The correlation between what's \textsl{"hard"} for the Controller and what for the Monitor can not be treated exhaustively in this study, however, plotting these curves showed an unexpected behaviour, motivating us to study more deeply this aspect in future works.

These curves are consistent with the data collected and analyzed, as the Safety-Monitor performs well with $C1$, while almost the same performances were observed for $C2\dots 4$, with slightly better performances with $C2$.\newpage

\begin{figure}[h!]
	\includegraphics[width=\textwidth]{img/precision-recall.png}
	\caption{Precision-Recall Curve for Checkpoints $C1\dots 4$, using values observed in each difficulty level}
\end{figure}
