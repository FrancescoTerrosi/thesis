...
The biggest issue in the training activity is the extremely time consuming, and it's not thinkable to train $n$ agents with $n$ strategies and wait for them to be trained to analyze which strategy gave the best result.
...
$C_{S1}$ performed significantly better than $C_{S0}$ both in terms of MTTF and MDTF. This reassure us on the fact that the System is actually travelling and higher MTTF is not a result of the car not moving, as happened for $C_{S2-4}$.
...
Comparing travelled distances charts, it looks like that $C_{S0}$ is the \textsl{"natural"} progress of $C4$. This makes sense, as it was trained with the same strategy. Looking at $C_{S1}$, if on one hand it had the best overall performances, there are scenarios in which it fails earlier than $C4$. This can be seen graphically looking at the right side of charts (approximately after scenario 130). It is true that differences are seen with $C4$ and $C_{S0}$ too, but small differences in running time may mean that the network is trying new solutions to avoid the events that resulted in a crash, while for $C_{S1}$ it seems that the Controller is not able anymore to handle some of the situations it previously handled.

\begin{minipage}[c]{\textwidth}
	\includegraphics[width=\textwidth]{simulations/genstupid/standard/distance-travelled-figure.pdf}
	\captionof{figure}{Meters travelled by $C_{S0}$ in each scenario at standard difficulty}
	\vspace{0.5cm}
	\includegraphics[width=\textwidth]{simulations/genpro/standard/distance-travelled-figure.pdf}
	\captionof{figure}{Meters travelled by $C_{S1}$ in each scenario at standard difficulty}
\end{minipage}
...
\begin{minipage}[c]{\textwidth}
	\includegraphics[width=\textwidth]{img/reliability-pro-stupid-comparison.png}
