...
CS0 FA FATICA DOVE C4 MENTRE CS1 SUPERA ALCUNE E FALLISCE ALTRE


Looking at $C_{S1}$, if on one hand it had the best overall performances, there are scenarios in which it fails earlier than $C4$. This can be seen graphically looking at the right side of charts (approximately after scenario 130). It is true that differences are seen with $C4$ and $C_{S0}$ too, but small differences in running time may mean that the network is trying new solutions to avoid the events that resulted in a crash, while for $C_{S1}$ it seems that the Controller is not able anymore to handle some of the situations it previously handled.
...
\begin{minipage}[c]{\textwidth}
	...
	\captionof{figure}{Reliability Function (minutes of operation) comparison between $C_{S0}$ and $C_{S1}$}

	\includegraphics[width=\textwidth]{img/reliability-pro-stupid-km-comparison.png}
	\captionof{figure}{Reliability Function (kilometers travelled) comparison between $C_{S0}$ and $C_{S1}$}
\end{minipage}
