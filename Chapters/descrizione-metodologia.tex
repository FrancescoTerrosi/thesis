...
Since we do not know the \textsl{real} amount of \textsl{True Negatives}, for the reason just described, some of the measures usually considered when assessing the prediction performances of a model could not be approximated. For the purpose of comparing the Monitor's performances for all the checkpoints, the following rates were approximated:
...
\begin{itemize}
	\item[1)] The reward function is now more punitive when the car hits something, while braking is slightly more rewarded, provided that the Car won't stop moving
	\item[2)] The Safety Monitor is attached to the system and the actions of the network are replaced with the Monitor's, if an alert is raised
	\item[3)] The network's action is replaced by the monitor's if it raises an alert and the reward function returns a positive value
	\item[4)] The training step is stopped and the network is given a \textsl{negative} reward if an alert is raised
\end{itemize}
...
As an additional step, we are going to treat the new Controllers as a Monitor for the runs recorded with the last checkpoint, to see how try to observe how the states space has changed.
